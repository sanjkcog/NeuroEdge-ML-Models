---
name: prp-commit
description: Natural-language commit procedure: interpret a plain-English target into a staged, single-line conventional-commit change set.
origin: NeuroEdge
---

# PRP — Commit

The full `prp-commit` procedure. It is single-sourced here so the exact same steps run whether a human invokes `/prp-commit` or a subagent reads this skill. The invoking command passes its `$ARGUMENTS` through as this procedure's input.
