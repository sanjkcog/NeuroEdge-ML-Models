...
| M2 | `verify` | `/dataset-verify --dest <dest>` → `ml-data-engineer` | `data/profile.json`, `data/splits/*.json` + `split_hash`, `data/portal_upload.zip` | **hard, human: data-verified** |
| M3 | `label` | `/auto-label --dest <dest>` (vision) / TS window rule | `data/label-manifest.md` | hard for vision; soft for TS |
| M4 | `synth` | `/synth-data --dest <dest>` — only if M2 said *accept as hold-out* or a class is rare | `data/synthetic-recipe.md` | none |
| M5 | `model-select` | `/model-select --dest <dest>` → `ml-modeler` | `model-select.md` | none |
| M6 | `model-build` | `/model-build --dest <dest>` → `ml-modeler`, then `ml-eval-reviewer` | `<arch>/train.py · eval.py · config.yaml · requirements · RUN_ON_GPU.md` | **hard: eval-methodology** |
| M7 | `train` | **external** — laptop GPU / AWS VM / platform trainer | `<arch>/runs/<run_id>/model-package/` | dependency-wait |
| M8 | `eval` | this command runs `<arch>/eval.py` on `data/splits/test.json` | `metrics.json` (`eval_split: held_out_test`) | **hard: KPIs + `beats_baseline`** |
| M9 | `return` | `POST /models/{use_case_id}/upload-return-package` | registration response saved to `<arch>/runs/<run_id>/return.json` | none |
| M10 | `model-card` | `ml-modeler` | `model-card.md` | none |
...
- **M2 data-verified (human):** `/dataset-verify` opens the gate; you record the decision it presents:
  `approve` → continue; `reject` with reason → re-enter M1 with the reason as a constraint (second real candidate
  at most) or M4 if the user chooses synthetic; `accept-as-hold-out` → M4 is mandatory.
- **M3:** vision — human review is the gate; TS — the window rule is recorded, no gate.
- **M6 eval-methodology:** `ml-eval-reviewer`'s report is the gate; any **leakage** or **wrong-metric** finding
  loops back to `ml-modeler` before the stage completes.
- **M8 KPIs:** compare `metrics.json` to the use case's targets (recall at the fixed FPR / mAP / …) and require
...
2. On `--resume`: look for `<arch>/runs/*/model-package/{model.onnx, meta.json, model_artifact.json, metrics.json}`.
   Found → `complete train --artifact <package>` and continue to M8. Not found → print exactly what is awaited and
...
`ml-model-package`); a 422 here is a defect in M6/M8 to fix, not a second opinion to argue with.
...
- Do not let any stage after M2 touch `data/splits/test.json` except `eval.py`.
